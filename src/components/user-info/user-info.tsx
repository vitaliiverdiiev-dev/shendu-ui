...
const UserInfo = React.forwardRef<HTMLDivElement | HTMLButtonElement, UserInfoProps>(
  ({ user, size = 'default', showRole = true, showChevron = false, className, asButton = false, ...props }, ref) => {
    const commonContent = (
      <>
        <Avatar className={cn(avatarSizeVariants({ size }))}>
          <AvatarImage src={user.avatarUrl} alt={user.name} />
          <AvatarFallback className={cn(avatarFallbackVariants({ size }))}>{getInitials(user.name)}</AvatarFallback>
        </Avatar>

        <div className="flex flex-col">
          <div className="flex items-center gap-2">
            <span className={cn(nameVariants({ size }))}>{user.name}</span>
            {showRole && user.role && <span className={cn(roleBadgeVariants({ size }))}>{user.role}</span>}
          </div>
          <span className={cn(emailVariants({ size }))}>{user.email}</span>
        </div>

        {showChevron && <ChevronDown className={cn(chevronVariants({ size }))} aria-hidden="true" />}
      </>
    );
